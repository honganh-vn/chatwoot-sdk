...
class ChatwootClientAuthServiceImpl extends ChatwootClientAuthService {
  ...
  Future<ChatwootConversation> createNewConversation(
    ...
    ///update contact for inbox with [inboxIdentifier] and passes [user] body to be linked to created contact
  @override
  Future<ChatwootContact> updateContact(String inboxIdentifier, ChatwootUser? user) async {
    try {
      final createResponse =
          await dio.put("/public/api/v1/inboxes/$inboxIdentifier/contacts", data: user?.toJson());
      if ((createResponse.statusCode ?? 0).isBetween(199, 300)) {
        //creating contact successful continue with request
        final contact = ChatwootContact.fromJson(createResponse.data);
        return contact;
